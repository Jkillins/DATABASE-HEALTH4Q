    ...
    DROP TABLE IF EXISTS assistant_referral;
    ...
    CREATE TABLE prescription_item (
        ...
        CONSTRAINT fk_item_medicine 
            FOREIGN KEY (medicine_id) REFERENCES medicine(med_id) 
            ON DELETE RESTRICT,
        -- Indexes
    ...
    -- ============ ASSISTANT REFERRAL TABLE ============
   CREATE TABLE assistant_referral (
    assistant_referral_id INT AUTO_INCREMENT PRIMARY KEY,
    appointment_id INT NOT NULL,
    referral_id INT NOT NULL,
    created_by INT NOT NULL,
    doctor_id INT NULL, -- Added
    specialist_name VARCHAR(255),
    clinic_location VARCHAR(255),
    notes TEXT,
    status ENUM('draft', 'submitted', 'received', 'completed', 'accepted', 'rejected') DEFAULT 'draft',
    urgency ENUM('routine', 'urgent', 'emergent') DEFAULT 'routine', -- Added
    followup_date DATE NULL, -- Added
    authorization_number VARCHAR(100) NULL, -- Added
    insurance_info VARCHAR(255) NULL, -- Added
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    submitted_at TIMESTAMP NULL,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    CONSTRAINT fk_asst_ref_appointment FOREIGN KEY (appointment_id) REFERENCES appointment(appointment_id) ON DELETE CASCADE,
    CONSTRAINT fk_asst_ref_referral FOREIGN KEY (referral_id) REFERENCES referral(referral_id) ON DELETE CASCADE,
    CONSTRAINT fk_asst_ref_user FOREIGN KEY (created_by) REFERENCES users(user_id),
    CONSTRAINT fk_asst_ref_doctor FOREIGN KEY (doctor_id) REFERENCES doctor(doctor_id),
    ...
    CREATE TABLE data_requests (
        ...
        -- Links to your existing users and doctor tables
        CONSTRAINT fk_patient FOREIGN KEY (patient_user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    ...
    CREATE INDEX idx_doctor_requests ON data_requests (doctor_id);
    CREATE INDEX idx_created_at ON data_requests (created_at);
